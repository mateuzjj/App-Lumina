...
export const Analytics: React.FC<AnalyticsProps> = ({ transactions }) => {
  ...
  // 1. Processamento Completo do Histórico (Todos os Meses)
  const historyData = useMemo(() => {
    const dataMap: Record<string, { date: Date, income: number, expense: number }> = {};
    
    // Agrupa todas as transações por Mês/Ano (YYYY-MM)
    transactions.forEach(tx => {
       // Considera apenas transações pagas para análise histórica precisa, ou todas? 
       // O prompt pede "receita - gastos", geralmente implicando fluxo de caixa real ou competência.
       // Vamos usar todas para ter uma visão geral do que foi lançado.
  ...
  // 2. Dados por Categoria (Top Offenders)
  ...
  return (
    <div className="space-y-8 pb-24 animate-fade-in pt-4">
      ...
      {/* 1. Cards de Totais Gerais (Lifetime) */}
      ...
      {/* 3. Detalhamento Mês a Mês (Lista) */}
      ...
      {/* 4. Categorias (Mantido como secundário) */}
