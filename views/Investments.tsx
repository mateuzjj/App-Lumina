...
import { TrendingUp, Plus, Trash2, DollarSign, Calculator, Calendar } from 'lucide-react';
...
export const Investments: React.FC<InvestmentsProps> = ({ investments, onAddInvestment, onDeleteInvestment }) => {
  ...
  const [newInv, setNewInv] = useState({ name: '', amount: '', rate: '10' }); // Default 10% a.a.
  ...
  // --- Lógica de Juros Compostos (Projeção Global) ---
  const projectionData = useMemo(() => {
    if (investments.length === 0) return [];

    const data = [];
    // Projetar para 10 anos (120 meses)
    for (let month = 0; month <= 60; month++) { // 5 anos para o gráfico ficar legível
      let totalAmount = 0;
      let totalInvested = 0;
      ...
      // Apenas adicionar pontos chave para o gráfico não ficar pesado, ou todos os meses
      if (month % 3 === 0) { 
        data.push({
            month,
            total: totalAmount,
