...
export const AddTransaction: React.FC<AddTransactionProps> = ({ initialDate, onSave, onCancel }) => {
  // --- Estados ---
  // rawValue armazena os centavos como inteiro. Ex: 150 = R$ 1,50
  ...
  // --- Lógica do Teclado Numérico Personalizado ---
  const handleDigitPress = (digit: string) => {
    setRawValue(prev => {
      const num = parseInt(digit);
      if (prev > 99999999) return prev; // Limite de segurança
      return (prev * 10) + num;
    });
  };
    ...
    <div className="fixed inset-0 bg-[#030712] z-50 flex flex-col animate-slide-up h-full overflow-hidden select-none">
      ...
      <div className="flex-1 flex flex-col items-center justify-center min-h-0 relative -mt-8 z-10">
         ...
         <div className="flex items-baseline gap-1 px-4 scale-125 origin-center">
             ...
             {/* Cursor piscante para indicar foco */}
      ...
      {/* 3. Área de Controle Inferior (Glassmorphism + Teclado) */}
      <div className="bg-[#111827] rounded-t-[36px] border-t border-white/5 pb-8 shadow-[0_-10px_60px_rgba(0,0,0,0.6)] flex flex-col h-[55%] backdrop-blur-3xl relative z-20">
        ...
        {/* Toolbar Intermediária (Data, Extras, Confirmar) */}
        <div className="px-6 flex items-center justify-between mb-4 mt-auto">
            <div className="flex gap-3">
                {/* Botão de Data com Picker Invisível */}
        ...
        <div className="grid grid-cols-3 gap-1 px-4 pb-safe h-64">
            ...
            <div className="h-full"></div> {/* Espaço Vazio */}
